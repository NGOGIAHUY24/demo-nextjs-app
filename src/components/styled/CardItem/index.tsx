...
//     handleDetail:
// }
export default function CardList({ data, handleDetail }) {
    // Call Store
    // const selectCars = useCarStore((state) => state.selectCars)
    const { brand, model } = splitTitle(data.title)
    return (
                ...
                <CardFooter className="flex flex-col text-small justify-between">
                    ...
                    <div className="grid grid-cols-2 ">
                        <div className="flex gap-2 mb-4">
                            ...
                            <span>{data.couch}</span>
                        </div>

                        <div className="flex gap-2">
                            <BatteryChargingIcon className="h-6 w-6" />
                            <span>{data.km}km (NEDC)</span>
                        </div>
                        ...
                        <div className="flex gap-0">
                            ...
                            <span>Dung tích cốp {data.trunk ?? "-"}L</span>
                        </div>
                    </div>
                </CardFooter>
            </Card>
